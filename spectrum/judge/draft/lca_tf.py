...
class LCA_Adavanced(TruthDiscoverer):
    def discover(self,
                 claims,
                 auxiliary_data=None,
                 epochs=1,
        ...
        return self._compute_trust_and_truth()

    def _compute_trust_and_truth(self):
        """verify the correctness"""
        with ed.tape() as tape:
            self.advanced_q()
        trust = dict()
        for s in range(self.n_sources):
            trust[f'z_trust_{s}'] = tape.pop(f'z_trust_{s}')
        truth = tape.copy()
        return trust, truth
    ...
    def _initialize(self):
        ...
        self._source_to_claims = self._compute_source_to_claims()
    ...
    def _compute_source_to_claims(self):
        source_to_claims = dict()

        for g in self.claims.groupby('source_id'):
            source_to_claims[g[0]] = list(g[1]['object_id'])
    ...
    def model(self):
        ...
        # p_trust
        for s in range(self.n_sources):
            ed.Bernoulli(name=f'z_trust_{s}', logits=self.honest_logits_p[s])
    ...
    def advanced_q(self):
        """a more complex variational model.
        Parameters
        ----------
        claims: pd.DataFrame
            a data frame that has columns [source_id, object_id, value]
        ...
        # q_trust
        for s in range(self.n_sources):
            ed.Bernoulli(name=f'z_trust_{s}',
                         logits=self.compute_z_trust_q_logits(s))

    def compute_z_trust_q_logits(self, s):
        """compute logits for a given source in q."""
        mode_probs = []
        for m in self._source_to_claims[s]:
            prob_s = tf.math.exp(tf.math.log_softmax(self.object_logits_q[m]))
            mode_s = prob_s[tf.math.argmax(prob_s)]
            mode_probs.append(mode_s)
        return tf.reduce_mean(mode_probs)